...
The `api` tag renders a grouped reference panel in the same shell as tabs and code blocks: pill tabs with entry counts, one compact signature row per entry. Author the data as a `yaml` code block inside the component — signatures, counts, and badges all derive from it, and editors and formatters treat the block as ordinary code.
...
Keep panels for reference sections. For a single narrated field inside prose, `field` remains the better fit.
