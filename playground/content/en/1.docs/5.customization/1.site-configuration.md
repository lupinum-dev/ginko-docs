...
`enabled: 'auto'` shows the default banner when the blog exists. Use an explicit boolean and localized copy for a release announcement. The dismissal is stored by `id`, so change the ID when the message must appear again.
