...
PNG social cards are enabled by default and generated during the build with `nuxt-og-image`. The built-in `GinkoDocs` template uses the page title, description, site name, product icon, and locale. The `nuxt` theme preset applies the shared Dark Spotlight composition with Nuxt green without another component setting.
...
Keep one approved square artwork file as the source. Generate the browser-specific formats with the package command:

```bash [Terminal]
pnpm exec ginko-docs-favicons --source public/product-icon.svg --out public
```

Ginko Docs links the generated files automatically. The command rejects non-square sources because browser masks and touch-icon crops would otherwise distort the mark.
