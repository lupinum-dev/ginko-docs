...
description: Keep content collections and Nuxt i18n aligned for English, German, or bilingual sites.
icon: lucide:languages
navigation:
  title: Localization
---

Ginko Docs supports English-only, German-only, and bilingual English-German sites. Select the content layout in `content.config.ts`, set the route-owning locale with `ginkoDocs.primaryLocale`, and keep Nuxt i18n aligned in `nuxt.config.ts`. These examples reuse the shared `site.json` value from the [quickstart]($docs/getting-started).
...
## Configure a German-only site

Keep German content directly under `content/dokumentation/`. Select German in both public configurations:
...
German remains unprefixed, so documentation starts at `/dokumentation`.
