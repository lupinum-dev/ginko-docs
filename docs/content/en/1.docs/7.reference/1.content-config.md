...
| `locales`          | `['en']`, `['de']`, or `['en', 'de']`    | `['en']` | Exact supported content layout; every other value throws at the boundary |
...
Single-language docs use the folder that matches their public route:

```text [English-only content]
content/
├── docs/**/*.md
...
English docs mount at `/docs`; German docs mount at `/dokumentation`. Blog posts mount at `/blog` in either mode. Collection-level i18n is left unset.
