...
Ginko Docs publishes the entry points below. Use these paths; other files inside the package are implementation details.
...
| `@lupinum/ginko-docs/authoring` | `ginkoDocsAuthoringKitSource` for supported visual components |
| `@lupinum/ginko-docs/component-kit` | Nuxt module with the visual kit renderers and styles |
| `@lupinum/ginko-docs/nuxt-config` | Nuxt configuration types |
| `@lupinum/ginko-docs/agent-docs` | Local Markdown entry for the installed documentation version |

Resolve `@lupinum/ginko-docs/agent-docs` from the application directory and read the file. It is documentation, not a JavaScript module. Follow its local page index when using an agent. The [installation prompt]($docs/getting-started/installation#use-a-coding-agent) preserves existing project instructions and adds only a missing package-docs pointer.
...
| Export                     | Kind           | Purpose                                                                 |
| -------------------------- | -------------- | ----------------------------------------------------------------------- |
| `ginkoDocsComponentTags`   | Runtime object | Maps all 34 authored tag names to Vue component names                   |
