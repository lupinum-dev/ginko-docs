...
| ------------------ | -------------------------------------------------------------------- |
| English only       | `content/docs/**/*.md`                                               |
| German only        | `content/dokumentation/**/*.md`                                      |
...
4. `locales` is exactly `["en"]`, `["de"]`, or `["en", "de"]` and agrees with Nuxt I18n;
5. the requested route starts with `/docs` in English, `/dokumentation` on a German-only site, or `/de/dokumentation` for German on an English-primary bilingual site.
...
Set the same production origin in Nuxt Site configuration, `ginkoDocs.site.url`, and Nuxt I18n's `baseUrl` when present. Ginko Content reads the canonical origin from Nuxt Site configuration. Clear generated output after changing route or locale configuration, then rebuild.
