...
`repository` enables edit and issue links. `feedback.enabled` adds the page-feedback control when Plausible is also configured. Feedback votes emit the `docs-feedback` event. See the [app configuration reference]($docs/reference/app-config) for every field and default.
