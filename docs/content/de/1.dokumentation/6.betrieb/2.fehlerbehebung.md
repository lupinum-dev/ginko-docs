...
| -------------------- | -------------------------------------------------------------------- |
| Nur Englisch         | `content/docs/**/*.md`                                               |
| Nur Deutsch          | `content/dokumentation/**/*.md`                                      |
...
4. `locales` ist exakt `["en"]`, `["de"]` oder `["en", "de"]` und stimmt mit Nuxt I18n überein.
5. Die angefragte Route beginnt auf Englisch mit `/docs`, auf einer rein deutschen Website mit `/dokumentation` oder auf einer zweisprachigen Website mit Englisch als Hauptsprache mit `/de/dokumentation`.
...
Setze dieselbe Produktions-Origin in Nuxt Site, `ginkoDocs.site.url` und `baseUrl` von Nuxt I18n, falls vorhanden. Ginko Content liest die kanonische Origin aus Nuxt Site. Lösche nach Änderungen an Routen oder Sprachen die generierte Ausgabe und baue neu.
