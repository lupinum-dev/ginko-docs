...
## Theme-Preset auswählen

Ein Preset stimmt Paletten, Flächen, Akzente der Landing Page sowie den Light und Dark Mode aufeinander ab:
...
Das `nuxt`-Preset verwendet die offizielle grüne Nuxt-Farbskala und Slate als Neutralpalette. Markenflächen verwenden helles Nuxt-Grün mit dunkelblauem Text. Text in normaler Größe verwendet im Light Mode ein dunkleres, barrierefreies Grün.

Verwende `preset: 'default'`, wenn du Neutral- und Primärpalette unabhängig auswählen willst. Ein anderes Preset steuert diese Paletten- und Flächen-Tokens gemeinsam. Zukünftige Ginko-Docs-Versionen können weitere Presets hinzufügen, ohne diese Konfigurationsform zu ändern.

## Integrierte Palette auswählen

Wähle neutrale Flächen und die Farbe für Primäraktionen unter dem Standard-Preset unabhängig voneinander in der App-Config:
...
## Syntaxhervorhebung wählen

Syntax-Tokenfarben werden zur Build-Zeit in `nuxt.config.ts` konfiguriert, getrennt von Laufzeit-Paletten und Codeblock-Darstellung in `app.config.ts`:

- `app.config.ts` — Neutral- und Primärpaletten sowie `codeBlocks`-Darstellung
- `nuxt.config.ts` — gebündelte Shiki-Themes für Light und Dark Mode
- Consumer-CSS — eigene Palettenwerte und semantische Token-Overrides

Ohne Konfiguration bleiben `light-plus` und `dark-plus` aktiv. Wenn du Syntaxhervorhebung konfigurierst, sind beide Themes erforderlich:
...
Kombiniere Syntax-Themes mit Laufzeit-Darstellung und eigenen Paletten in der App-Config:
...
Shiki steuert nur Tokenfarben. Flächen, Rahmen und Hintergründe von Codeblöcken bleiben über CSS und `theme.codeBlocks` beim Consumer. Siehe [App-Konfiguration]($docs/reference/app-config) für das Verhalten von `codeBlocks`.
...
Die gemeinsame Kopfzeile nutzt einen binären Hell/Dunkel-Schalter, der dem Betriebssystem folgt, bis Leserinnen und Leser explizit wählen. Um `System`, `Hell` und `Dunkel` wieder anzubieten, ersetze `ModeToggle` in einer eigenen `SiteHeader`-Komponente. Wenn du eine ältere `SiteLocaleSwitcher`-Kopie mit den Varianten `pill`, `menu-tile` oder `segmented` verwendest, passe sie an die aktuellen Layer-Props `dropdown` und `menu-row` an.
...
Ein Wechsel von der aktuellen Darstellung speichert eine explizite Hell- oder Dunkel-Präferenz. Ein weiterer Klick stellt die Betriebssystem-Vorgabe wieder her, wenn das Ziel der OS-Einstellung entspricht.
...
<Files :annotations='{"app/layouts/docs.vue":"Dokumentations-Shell","app/pages/index.vue":"Startseite"}'>
- `app`
  - `layouts`
    - `docs.vue`
  - `pages`
