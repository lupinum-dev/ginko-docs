...
`repository` aktiviert Links zum Bearbeiten und Melden. `feedback.enabled` fügt die Seitenbewertung hinzu, wenn Plausible ebenfalls konfiguriert ist. Bewertungen senden das Ereignis `docs-feedback`. Die [Referenz zur App-Konfiguration]($docs/reference/app-config) beschreibt alle Felder und Standardwerte.
