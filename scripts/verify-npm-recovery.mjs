...
export const createRegistryVerificationRecord = async ({
  ...
  if (registryShasum !== null) {
    ...
    // For a DSSE bundle, Sigstore's options are the second argument. Passing an
    // explicit undefined payload would discard these identity constraints.
...
const fetchAttestations = async (metadata) => {
  if (!metadata?.url) fail("The existing npm version has no provenance URL.");
  const url = new URL(metadata.url);
  if (url.origin !== REGISTRY_URL) fail("The npm provenance URL is outside the registry.");
