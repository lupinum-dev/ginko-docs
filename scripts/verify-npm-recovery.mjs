...
export const REPOSITORY_URL = "https://github.com/lupinum-dev/ginko-docs";
...
const sourceFromStatement = (statement) => {
  const dependencies = statement.predicate?.buildDefinition?.resolvedDependencies ?? [];
  const sourceSha = dependencies.length === 1 ? dependencies[0]?.digest?.gitCommit : undefined;
...
const verifyProvenance = async ({
  ...
  await (verifyBundle ?? loadSigstoreVerifier())(bundle, sigstorePolicy());
...
export const createRegistryVerificationRecord = async ({
  ...
  if (registryShasum !== null) {
    ...
    if (sourceSha !== manifest.commit)
      fail("The registry source differs from the artifact source.");
...
const verifyArtifact = async () => {
  ...
  if (
    manifest.packageName !== "@lupinum/ginko-docs" ||
  ...
  const verifyBundle = loadSigstoreVerifier();
